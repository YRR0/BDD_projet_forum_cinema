...
\! echo Requête 1 : logins des utilisateurs, textes des publications, noms des catégories;
...
\! echo Requête 2.1 : noms des artistes ayant le même sexe et le même nombre d Oscars;
...
\! echo Requête 2.2 : films du même genre mais avec des titres différents;
...
\! echo Requête 3 : logins des utilisateurs suivis par au moins un autre utilisateur;
SELECT u.logins
FROM projet.Utilisateur u
WHERE EXISTS (
    SELECT *
...
\! echo Requête 4 : logins des utilisateurs et nombre de followers;
...
-- Cette requête sélectionne les logins des utilisateurs qui sont suivis par au moins un autre utilisateur.
\! echo Requête 5 : logins des utilisateurs suivis par au moins un autre utilisateur;
SELECT logins
...
\! echo Requête 6.1 : artistes ayant remporté plus de 9 Oscars;
...
-- Cette requête sélectionne les réalisateurs avec plus de 1 film et une moyenne d'entrée supérieure à 100 000 
\! echo Requête 6.2 : Artistes avec plus de 1 films et une moyenne d entrees supérieure à 100 000 
SELECT a.nom, COUNT(f.titre) AS nb_films, AVG(f.nb_Entrees) AS moy_entrees
FROM projet.Artiste a
JOIN projet.Film f ON a.ID_Artiste = f.ID_Realisateur
GROUP BY a.nom
HAVING COUNT(f.titre) > 1 AND AVG(f.nb_Entrees) > 100000;

-- 7. Calcul de deux agrégats :
--Cette requête calcule les moyennes des nombres maximums et minums d'entrées réalisées par chaque genre de film.
\! echo Requête 7 : Calcul de la moyenne des maximums et des minimums des entrées des films par genre;
WITH Genre_Stats AS (
    SELECT g.nom AS genre, MAX(f.nb_Entrees) AS max_entrees, MIN(f.nb_Entrees) AS min_entrees
    FROM projet.Film f
    JOIN projet.Genre g ON f.genre = g.nom
    GROUP BY g.nom
)
SELECT AVG(max_entrees) AS avg_max_entrees, AVG(min_entrees) AS avg_min_entrees
FROM Genre_Stats;
...
\! echo Requête 8 : nom des artistes et nombre total de réactions associées;
...
\! echo Requête 9.1 : logins des utilisateurs qui n ont aucun follower (sous-requêtes corrélées);
SELECT u.logins
FROM projet.Utilisateur u
WHERE NOT EXISTS (
    SELECT 1
...
-- Avec agrégation :
-- Cette requête sélectionne les logins des utilisateurs qui n'ont aucun follower.
\! echo Requête 9.2 : logins des utilisateurs qui n ont aucun follower (agrégation);
SELECT u.logins
FROM projet.Utilisateur u
...
\! echo Requête 10.1 : genres à la fois parent1 et parent2 (INTERSECT);
SELECT R.parent1 
...
\! echo Requête 10.2 : genres à la fois parent1 et parent2 (DISTINCT);
...
\! echo Requête 11 : profondeur des commentaires à partir d un commentaire spécifique;
WITH RECURSIVE commentairesRecursifs AS (
   ...
   UNION ALL
   SELECT c.ID_commentaire , c.auteur, c.origine_c 
   FROM projet.commentaire AS c, commentairesRecursifs
...
\! echo Requête 12 : dix lieux d événements ayant le plus de succès par mois en 2023;
...
WHERE DATE_PART('year', e.date_e) = 2023
...
\! echo Requête 13 : recommandation d événements basés sur ceux suivis par les amis de colin59;
WITH Amis AS (
    SELECT suivi
    FROM projet.Follow 
...
\! echo Requête 14 : événements auxquels sydneylee est intéressé;
SELECT ID_Evenement, titre, date_e, lieu
FROM projet.Evenement
...
\! echo Requête 15 : recommandations de publications basées sur les sujets consultés par michael17;
SELECT p.*
...
\! echo Requête 16 : calcul d un score de recommandation pour les événements;
WITH UserEvents AS (
    SELECT ID_Evenement
    FROM projet.Interesse
    WHERE ID_Utilisateur = 'sydneylee'
...
\! echo Requête 17 : recommandation des n meilleurs événements pour sydneylee;
WITH UserEvents AS (
    SELECT ID_Evenement
    FROM projet.Interesse
    WHERE ID_Utilisateur = 'sydneylee'
...
\! echo Requête 18 : recommandation de films aimés par les utilisateurs suivis par sydneylee;
...
\! echo Requête 19: les événements suivis par l utilisateur 'nicholasbryant'
SELECT e.*
FROM projet.Evenement e
JOIN projet.Interesse i ON e.ID_Evenement = i.ID_Evenement
...
\! echo Requête 20: recommande des publications basées sur les publications aimées par l'utilisateur et les utilisateurs qu'il suit.
WITH UtilisateursSuivis AS (
    SELECT suivi
    FROM projet.Follow
    WHERE suiveur = 'sydneylee'
